...
# Static checks
npm run lint                     # ESLint (next lint, scoped to src/)
...
`next.config.js` sets `typescript.ignoreBuildErrors: true`, and CI runs `type-check` as **non-blocking** because ~470 pre-existing type errors in page files were left behind by the V2 rework. A passing build does NOT mean the types are clean. Don't add new type errors; run `npm run type-check` on files you touch and compare against baseline rather than expecting zero errors.
...
Next.js API routes in `src/app/api/` (tokens, analytics, leaderboard, ipfs, push) do server-side RPC aggregation and IPFS uploads. The server's contract ABIs are hand-maintained copies in `server/src/abis/` — keep them in sync after contract changes.
...
Vercel deploys via `.github/workflows/deploy.yml`: pushes to main → production, other branches → preview URLs (skips gracefully without `VERCEL_TOKEN`).

## Reference docs

The repo has extensive markdown docs at the root. Most useful: `AGENTS.md` (agent-oriented overview), `BONDING_CURVE_MATH.md` (curve math spec), `TESTING_GUIDE.md`, `CONTRACT_CONFIGURATION.md`, `TROUBLESHOOTING.md`, `EMERGENCY_RUNBOOK.md`. `TECHNICAL_DEBT.md` tracks known placeholders (e.g. holder counts return 0). Everything under `docs/archive/` is historical — don't treat it as current state.

Project-specific Claude Code skills live in `.claude/skills/` (smart-contract-deployment, web3-testing, kaspump-token-launch, webapp-testing).
