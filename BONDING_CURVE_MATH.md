...
> ⚠️ **OUTDATED — describes the pre-V2 linear/quadratic curve.**
> The V2 rework replaced this with a **sigmoid** curve implemented as a
> 31-anchor piecewise-linear table in `contracts/libraries/BondingCurveMath.sol`
> (generated by `scripts/generate-sigmoid-anchors.js`). The current curve does
> **not** use the `Math.mulDiv` linear-price formula documented below, and the
> pricing is piecewise-linear between anchors rather than a closed-form
> integral. Treat everything below as historical background for the legacy
> curve; see `BondingCurveMath.sol` and the `project-conventions` skill for the
> shipped math.
